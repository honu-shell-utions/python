...
def dart_value(dart):
    ...
    if rankStr == 'S':
        ...
        return 3*val
################################################################################
def checkout_value(checkout):
    total = 0
    for dart in checkout.split(','):
        total += dart_value(dart)
    return total
################################################################################
def load_the_lists():
    darts = []
    ...
    #load the darts
    for k in range(1,21):
        darts.append('S'+str(k))
    for k in range(1,21):
        darts.append('D'+str(k))
        doubles.append('D'+str(k))
    for k in range(1,21):
        darts.append('T'+str(k))
    darts.append('S25')
    darts.append('D25')
    doubles.append('D25')

    #create the set checkouts
    checkouts = set(doubles)

    #create the two and three throw games
    for db in doubles:
        for a in darts:
            checkouts.add((a+','+db))
            for b in darts:
                if a < b:
                    checkouts.add(a+','+b+','+db)
                else:
                    checkouts.add(b+','+a+','+db)                   
    return checkouts        
################################################################################
start = time()
MAX = 100
checkouts = load_the_lists()
total = 0
for chk in checkouts:
    game_value = checkout_value(chk)
    if game_value < MAX:
        total += 1
print('Solution:',total,'Runnin Time:',time()-start)
################################################################################
#solution: 38182
################################################################################
