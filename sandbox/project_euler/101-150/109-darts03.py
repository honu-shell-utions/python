...
##a "checkout" and the highest checkout is 170: T20 T20 D25
...
##  S2	S2  D1
##
##Note that D1 D2 is considered different to D2 D1 as they finish
##on different doubles. However, the combination S1 T1 D1 is considered
...
################################################################################
def winning_game(starting_score,game):
    game_total = 0
    for k in range(len(game)):
        game_total += game[k][1]
    if game_total == starting_score:
        return True
    else:
        return False        
################################################################################    
possible_scores = []
for k in range(1,21):
    possible_scores.append(['S'+str(k),k])
for k in range(1,21):
    possible_scores.append(['D'+str(k),k*2])
for k in range(1,21):
    possible_scores.append(['T'+str(k),k*3])
possible_scores.append(['S25',25])
possible_scores.append(['D25',50])

temp_possible_games = list(chain.from_iterable(product(possible_scores, repeat=r) for r in range(1,4)))

temp_winning_games = []
for tpg in temp_possible_games:
    size = len(tpg)
    temp = tpg[size-1][:1]
    if 'D' in str(temp):
        temp_winning_games.append(tpg)

possible_winning_games = []
for game in temp_winning_games:
    if len(game) == 3:
        t1,t2,t3 = game
        if (t2,t1,t3) not in possible_winning_games:
            possible_winning_games.append(game)
    else:
        possible_winning_games.append(game)

total = 0
for score in range(2,100):
    for wg in possible_winning_games:
        if winning_game(score,wg):
            total += 1
