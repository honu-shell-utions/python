...
class Problem():
    def __init__(self):
        ...
        self.pi_memo = {1: 0}
    #  ------------------------------------------------------------------------
    def primerange(self,b,a=2):
        is_prime = [True]*(b-a+1)                   # partial sieve
        if a < 2: is_prime[:2-a] = [False]*(2-a)    # 0 and 1 are not primes
        for p in (2,*range(3,int(b**0.5)+2,2)):     # scan divisors up to √b
            base = (p - a%p)%p + p*(p>=a)           # first multiple in partial sieve
            is_prime[base::p] = [False]*len(is_prime[base::p])      # flag non-primes
        return [p for p,prime in enumerate(is_prime,a) if prime]    # return primes
    #  ------------------------------------------------------------------------
    def init_primes_and_p_lists_and_bound(self):
        self.primes_list = self.primerange(self.n)
...
def main():
    ...
    u = Problem()
    u.solve()
    print('Run-Time:', perf_counter() - start, 'sec')
#  -----------------------------------------------------------------------------
main()
#  solution: 172023848
#  -----------------------------------------------------------------------------
