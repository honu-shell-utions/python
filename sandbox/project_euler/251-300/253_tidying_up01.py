...
## Tidying up
## Problem 253
## A small child has a “number caterpillar” consisting of forty
## jigsaw pieces, each with one number on it, which, when connected
## together in a line, reveal the numbers 1 to 40 in order.
## 
## Every night, the child's father has to pick up the pieces of the
## caterpillar that have been scattered across the play room. He picks
...
def find_segments(p):
    d = dict()
    ranges =[]
    for k, g in groupby(enumerate(p), lambda x: x[0]-x[1]):
        group = (map(itemgetter(1), g))
        group = list(map(int, group))
        if group[0] != group[-1]:
            ranges.append((group[0], group[-1]))
            d[(group[0], group[-1])] = group[-1] - group[0]
    return ranges, d
#-------------------------------------------------------------------------------
num_pieces = 6
pieces = [i for i in range(1, num_pieces + 1)]
perms = permutations(pieces,num_pieces)
d_segs = dict()
M = 2
for p in perms:
    lst, d_segs = find_segments(p)
    for k,v in d_segs.items():
        if v == M:
            print(p,d_segs,M,k)
