# Fact: When a number is factorized as a product of powers of unique primes,
# the number of factors
# the number has is equal to the product of {each exponent plus one}.
# That is, if n = p_1^k_1 * p_2^k_2 * ... * p_m^k_m, then n has exactly
# (k_1 + 1) * (k_2 + 1) * ... * (k_m + 1) factors.
# 
# Define any natural number n to be a "candidate" iff it has exactly 2^500500 factors
# (including 1 and n).
# 
# First examine n = 2^1 * 3^1 * 5^1 * 7^1 * ... * 7376497^1 * 7376507^1, which is the
# product of the first 500500 prime numbers. n has (1+1) * (1+1) * ... * (1+1) =
# 2 * 2 * ... * 2 = 2^500500
# factors, as desired. This candidate n is thus an upper bound on the answer we're
# looking for.
# 
...
#  -----------------------------------------------------------------------------
start = time()
print(f'Solution: {euler_500()}, Run-Time: {time()-start}')
#  -----------------------------------------------------------------------------
#  solution: 35407281
#  -----------------------------------------------------------------------------
