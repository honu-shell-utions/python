# ------------------------------------------------------------------------------
# 189_tri-coloring_tri_grid.py
# tri-colouring a triangular grid
#
# Consider the following configuration of 64 triangles:
#
# We wish to colour the interior of each triangle with one of three colours:
# red, green or blue, so that no two neighbouring triangles have the same
# color. Such a colouring shall be called valid. Here, two triangles are said
# to be neighbouring if they share an edge.
#
# Note: if they only share a vertex, then they are not neighbours.
#
# For example, here is a valid colouring of the above grid:
# A colouring C' which is obtained from a colouring C by rotation or 
# reflection is considered distinct from C unless the two are identical.
# 
# How many distinct valid colorings are there for the above configuration?
# ------------------------------------------------------------------------------
from time import time
import itertools
# ------------------------------------------------------------------------------
start = time()
K = 3
N = 8
colors = range(0, K)
DP = [None, {(i,): 1 for i in colors}]
# ------------------------------------------------------------------------------
def flip(a):
    number = []
    for i in a:
        digit = [d for d in colors if d != i]
        number.append(digit)
    return itertools.product(*number)
# ------------------------------------------------------------------------------
def double_flip(a):
    number = [[d for d in colors if d != a[0]]]
    for i in range(0, len(a) - 1):
        number.append([d for d in colors if d != a[i] and d != a[i + 1]])
    number.append([d for d in colors if d != a[len(a) - 1]])
    return itertools.product(*number)
# ------------------------------------------------------------------------------
for i in range(2, N + 1):
    DP.append({})
    for a, v in DP[i - 1].items():
        for a_prime in flip(list(a)):
            for b in double_flip(a_prime):
                bt = tuple(b)
                if bt not in DP[i]:
                    DP[i][bt] = 0
                DP[i][bt] += v

c = 0
for b, v in DP[N].items():
    c += v

print(f'Solution: {c}, Run-Time: {time()-start}')
# ------------------------------------------------------------------------------








# ------------------------------------------------------------------------------








# ------------------------------------------------------------------------------
# solution: 10834893628237824
# ------------------------------------------------------------------------------
