...
https://mathnet.mit.edu/explorer.html?p=usa_2022_a73c8b
...
for _ in range(throws):
    x = uniform(0,20)
    y = uniform(0,8)
    in_circle_A = (x-5)**2+(y-5)**2 < 25
    in_circle_B = (x-17)**2+(y-4)**2 < 25
    if not in_circle_A and not in_circle_B:
        plot(x,y,'.')
        hits += 1
area = hits/throws * 160
title(f'The red area {area:0.8f}.')
axis('off')
axis('equal')
show()
